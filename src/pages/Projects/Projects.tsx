...
const Document: React.FC<DocumentProps> = ({ file,title,fold }) => {
  ...
  const [projectStructure, setDocumentStructure] = useState<DocumentData[]>([]);
  ...
  useEffect(() => {
    const fetchDocumentStructure = async () => {
      try {
        ...
        const response = await fetch(`${basePath}data/projectStructure.json`); // Prepend basePath
        if (!response.ok) {
  ...
  const handleFilterChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (value.trim() === '') {
      setFilteredDocs(projectStructure);
    } else {
      const filtered = projectStructure.filter(doc => 
